...
La tecla principal, de ahora en adelante `X`, es `Mod4`. Cabe destacar que, por defecto en el sistema de suckless es `X` (`Alt`).
...
`X-t` - **tiled layout** - re-arranges windows into the dynamically-tiled master/stack configuration (if not already).

![tiled_layout](misc/tiled_layout.png)

`X-m` - **monocle layout** - very handy, single window fullscreen layout - whichever window is focused is pushed to the front and viewed fullscreen. Switch windows by focusing them (see below).

![monocle_layout](misc/monocle_layout.png)

`X-f` - **floating layout** - allows windows to overlap and be moved and sized manually.
...
`X-space` - **toggle layouts** - this toggles between the most current two layouts.

`X-Shift-space` - **toggle window layout** - for the focused window, this toggles between tiled and floating states.
...
`X-Shift-return` - **open terminal** - this specifically opens st, if you want it to open another terminal, you’ll need to customize config.h.

![new_term](misc/new_term.png)

`X-p` - **open dmenu** - runs the tiny dynamic menu called dmenu in dwm’s top bar.
...
`X-Shift-c` - **close focused window**

![close_window](misc/close_window.png)

#### Switching window focus

`X-j` - **focus next window** - in the tiled layout, this goes in clockwise order.
...
`X-k` - **focus previous window** - in the tiled layout, this goes in counter/anti-clockwise order.
...
`X-i` - **increment master** - increase the number of split windows in the master area (will move windows (if any) out of the stacking area as needed).

![master_increase](misc/master_increase.png)

`X-d` - **decrement master** - decrease the number of split windows in the master area (will move windows (if any) back to the stacking area as needed).

![master_decrease](misc/master_decrease.png)

`X-l` - **increase master area size** - make the master area physically larger.

![master_bigger](misc/master_bigger.png)

`X-h` - **decrease master area size** - make the master area physically smaller.

![master_smaller](misc/master_smaller.png)

`X-Return` - **zoom** - toggle window between the master and stack areas: if the focused window is in the stack, it is moved to the master area and the previous master window is put at the top of the stack; or if the focused window is in the master area, it is put on the stack.
...
`X-Shift-[1..n]` - **apply tag to window** - assigns the tag number to the currently-focused window (like moving a window to a different desktop).
...
`X-[1..n]` - **view tag** - displays all windows with tag number (like viewing a particular desktop).

    Example: Alt-2 displays all windows assigned to tag '2'.

![view_tagged_windows](misc/view_tagged_windows.png)

`X-Tab` - **toggle tags** - this is like switching between the two most recent desktops.

`X-Shift-0` - **apply all tags to window** - this is like saying "put this window on all desktops."

![assign_all_window_tags](misc/assign_all_window_tags.png)

`X-Control-Shift`-[1..n] - **toggle tag n** - this adds or removes the tag number for the focused window (like adding or removing the window from a particular desktop).

`X-0` - **view all windows** - regardless of tag (like viewing all desktops at once).

![view_all_windows](misc/view_all_windows.png)

`X-Control-[1..n]` - **banish tags** - Add/remove all windows with nth tag to/from the view (only makes sense in the context of the above option of viewing all windows).

#### Quitting dwm

`X-Shift-q` - **quit dwm** - You shall return whence you came!

#### Toggle the top bar

`X-b` - **toggle the top bar** - It is like a magic appearing/disappearing act! What fun!

![toggle_bar](misc/toggle_bar.png)

#### Mousing around

I’ll be the first to admit that the mouse has its place. Sometimes you just have to click on a floating window and drag it away to paradise. dwm handles this intuitively (once you know to hold down the X key).

![mouse_controls](misc/mouse_controls.png)

`X-Button1` - **move window** - Hold and drag to move the window. If the window was tiled, it will be floated (the rest of the layout will remain tiled - it’s the best of both worlds!)

`X-Button2` - **toggle layout** - toggles the focused window between the floating and tiled state.

`X-Button3` - **resize window** - Hold and drag to resize the window. As with moving, if the window was tiled, it will be floated.
...
`X-,` - **focus previous screen**

`X-.` - **focus next screen**

`X-Shift-,` - **send window to previous screen**

`X-Shift-.` - **send window to next screen**
